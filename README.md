...
Cette valeur maximale est de l'ordre de $1$, mais peut prendre des valeurs très différentes selon la méthode de résolution utilisée (par défaut **rk4** dans l'exemple présenté) et selon les conditions aux limites.
...
les programmes en **Python**. L'attribut **self.interval** détermine cette fois de façon exacte la cadence de rafraîchissement
